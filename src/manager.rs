...
impl AltManager {
    ...
    pub fn fetch_alt_for_stage(&self, stage: Hash40, alt: usize) -> Option<usize> {
        ...
        match resolved {
            Some(info) => log::info!(
                "[stage-alts] fetch_alt_for_stage online_or_bgm: stage={} selected_index={} resolved_slot={}",
                stage.pretty(),
                alt,
                info.slot_value
            ),
            None => log::warn!(
                "[stage-alts] fetch_alt_for_stage online_or_bgm: stage={} selected_index={} resolved_slot=None",
                stage.pretty(),
                alt
            ),
        }

        resolved.map(|alt| alt.slot_value)
    }
}
